...
const EventForm = () => {
  ...
  useEffect(() => {
    ...
    const savedRole = user?.role?.trim().toLowerCase(); // ✅ FIXED
    console.log('Saved role:', savedRole); // Optional for debugging
    setRole(savedRole);
 ...
 const handleSubmit = async (e) => {
  ...
  Object.entries(formData).forEach(([key, value]) => {
    if (key === 'time') {
      data.append('time', `${formData.time} ${formData.period}`);
    } else if (key !== 'period') {
  ...
  // ✅ Append organizerId from logged-in user
  const user = JSON.parse(localStorage.getItem('user'));
  if (user?._id) {
    data.append('organizerId', user._id);  // 🔥 THIS WAS MISSING!
  }
